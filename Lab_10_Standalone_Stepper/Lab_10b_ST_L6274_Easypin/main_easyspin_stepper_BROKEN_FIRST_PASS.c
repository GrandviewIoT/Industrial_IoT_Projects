// 09/06/15 - TIM3_IRQ from PWM1_TIM3 is being invoked.
//            EXTI15_10_IRQHandler from 6474 IRQ is _NOT_ being invoked
...
// --- begin --- The following is equivalent to Easyspin_Begin() code
    ...
    pin_Low (L6474_RESET_PIN);                     // turn on RESET
...
/// uwPeriod = (SystemCoreClock / 20000 ) - 1;       // set period = 20 K Hz - WVD LOGIC
    ...
    timer_Set_Prescalar(L6474_PWM_1_MODULE, (TIMER_PRESCALER - 1), 0);
    if (rc == HAL_OK)
       rc = pwm_Init (L6474_PWM_1_MODULE, uwPeriod, 
                      TIMER_NO_AUTO_PRESCALING);     // actual rate on scope = 40 KHz !
